...
public class GeneratorUtils {
    ...
    public void doGenerate(AnnotationEnum generateType) {
        WriteCommandAction.runWriteCommandAction(project, () -> {
            ...
            if (selection) {
                this.generateSelection(psiClass, selectionText, isController, generateType);
                return;
            }
        ...
        for (PsiElement tmpEle : psiClass.getChildren()) {
            if (tmpEle instanceof PsiComment) {
                ...
                if (isController) {
                    ...
                    if (Objects.equals(AnnotationEnum.SWAGGER,generateType)) {
                        annotation = "ApiModel";
...
//                    String.format("@ApiImplicitParam(paramType = \"%s\", dataType = \"%s\", name = \"%s\", value = \"\", required = %s)",paramType, dataType, psiParameter.getNameIdentifier().getText());
     ...
     */
    private void generateFieldAnnotation(PsiField psiField, AnnotationEnum generateType) {
        PsiComment classComment = null;
